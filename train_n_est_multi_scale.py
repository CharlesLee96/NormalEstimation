...
def train_pcpnet(opt):
    ...
    os.system('cp train_n_est_multi_scale.py %s' % (log_dirname))  # backup the current training file
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            # forward pass
            pred, beta_pred, weights, trans, trans2, neighbor_normals, bias, points_new = model(points)   #, _,_,_,_,_,_
    ...
    for oi, o in enumerate(outputs):
        if o == 'unoriented_normals' or o == 'oriented_normals':
            ...
            if o == 'unoriented_normals':
                ...
                '''pred_weights = point_weights.squeeze()

                thres_d = 0.05 * 0.05
                # normal_dis = torch.bmm(o_target.unsqueeze(1), pcl_in.transpose(2, 1)).pow(2).squeeze()  # (B, N)
                normal_dis = torch.bmm(o_target.unsqueeze(1), pcl_in).pow(2).squeeze()  # (B, N)
...
def get_model(opt, log_dirname):
    ...
    #os.system('cp %s %s' % ('AdaFit_multi_scale.py', os.path.join(log_dirname, 'DeepFit.py')))  # backup the current model
    os.system('cp AdaFit_multi_scale.py %s' % (log_dirname))
    return model
